...
export function Overview({ onNavigate }) {
  ...
  useEffect(() => { inbox.run(8) }, [inbox.run])

  const orders = inbox.result?.orders || []
  ...
  return <>
    ...
    <Panel title="Recent order notifications" description="Orders received from Noon's FBPI order-sync feed." actions={<button className="text-button" onClick={() => inbox.run(8)}><Icon name="refresh" size={15} /> Refresh</button>}>
      <ErrorNotice error={inbox.error} />
      {inbox.busy ? <div className="loading-state">Loading order notifications…</div> : <DataTable columns={columns} rows={recentRows} rowKey={(row) => row.fbpi_order_nr} emptyTitle="No order notifications yet" />}
    </Panel>
  </>
}
